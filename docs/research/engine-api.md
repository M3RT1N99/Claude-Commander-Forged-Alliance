# Engine Lua API — which binding goes into which VM

**Generated by [scripts/dump-engine-api.ts](../../scripts/dump-engine-api.ts) from the
IDA decompilation. Do not care for by hand.**

Each engine Lua binding is a `Moho::CScrLuaInitForm luadef_*`. The field
`mPrevDef` reveals which Lua VM it is registered in - and thus that
...
That's why the sim doesn't know `_c_CreateCursor` and the UI doesn't know `CreateUnit`. Who
...
**453 bindings** — 200 globals, 23 classes.
...
**626 bindings** — 133 globals, 27 classes.
